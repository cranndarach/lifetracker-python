...
```{r}
plot(combo$SinceWakeup, combo$Spoons, type="n",
     xlab="Time Since Waking Up (hours)",
     ylab="Spoons",
     main="Effect of time since waking up on spoons\nat three levels of previous night's sleep")
...
legend(12, 83, legend=c("Low", "Medium", "High"), lty=c(1,1), lwd=2.5, 
       col=c("red2", "goldenrod1", "green3"))
# :\nlow (blue), average (green), and high (red)
# require(knitr)
# graph.colors <- c("blue","green","red")
# graph.levels <- c("Low","Average","High")
# kable(cbind(graph.colors, graph.levels),col.names=c("Color","Previous Night's Sleep"),
#       row.names=F, caption="Legend")
```
